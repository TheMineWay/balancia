...
import {
	DebtOriginCreatedEvent,
	DebtOriginDeletedEvent,
...
export class DebtOriginService {
	...
	async userSetToDebt(
		...
		await this.databaseService.db.transaction(async (transaction) => {
			...
			const existingOrigins = await this.debtOriginRepository.findByDebtId(
				debtId,
				{
					transaction,
				},
	...
	async userGetByDebt(userId: UserModelId, debtId: DebtModel["id"]) {
